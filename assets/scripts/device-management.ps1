...
$DevicesInError = foreach($Device in $DeviceState){
  $Errortext = switch($device.ConfigManagerErrorCode){
    ...
    default {"Found device error. Error code: $($device.ConfigManagerErrorCode)"}
  }
...
$videoDriver = Get-WmiObject Win32_VideoController

if ($videoDriver.Description -eq "Microsoft Basic Display Adapter") {
  Write-Output "WARNING: Using Windows Generic Video driver"
}
...
if(!$DevicesInError){
  ...
  Write-Output "$(Out-String -InputObject $DevicesInError)"
  Write-Result
  exit 2
}
